...
function getHttps(url) {
  return new Promise((resolve, reject) => {
    https.get(url, { headers: { 'User-Agent': 'Mozilla/5.0' }, timeout: 15000 }, (r) => {
      const c = [];
      r.on('data', (d) => c.push(d));
      r.on('end', () => resolve({ status: r.statusCode, len: Buffer.concat(c).length }));
    }).on('error', reject);
  });
}
...
(async () => {
  ...
  try {
    const remote = await getHttps('https://www.shader.se/');
    console.log('shader.se', remote.status, remote.len);
  } catch (e) {
    console.log('shader.se FAIL', e.message);
  }
