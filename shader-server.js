...
/** true = shader.se'ye canlı bağlanmaz, yalnızca yerel cache/static */
...
function fetchRemote(url) {
  if (STANDALONE) {
    return Promise.reject(new Error('Standalone mod: uzak sunucu devre disi'));
  }
...
const SCRIPT_VER = '105';

/** false = orijinal Shader projeleri (karusel calisir). HEIP -> Lider Teknik cevirisi ayri yapilir. */
...
function serveWorkRsc(slug, res) {
  ...
  for (const file of candidates) {
    ...
    body = applyTranslationsBuffer(Buffer.from(body, 'utf8'), 'html').toString('utf8');
    res.writeHead(200, {
      'Content-Type': 'text/x-component',
      'Cache-Control': 'no-store, no-cache, must-revalidate',
      Pragma: 'no-cache',
...
async function serveWork(pathname, query, req, res) {
  ...
  if (isRscRequest(req)) {
    ...
    res.writeHead(404, { 'Content-Type': 'text/plain; charset=utf-8' });
    res.end('Proje verisi yerelde yok. Calistirin: npm run mirror');
    return true;
  }
  ...
  res.end(errorPage(`Proje sayfasi yerelde yok (${slug}). Calistirin: npm run mirror`));
  return true;
}
...
async function refreshCss(html) {
  ...
  try {
    ...
    if (fs.existsSync(localCss)) {
      text = fs.readFileSync(localCss, 'utf8');
    } else if (!STANDALONE) {
      const css = await fetchRemote('https://www.shader.se' + cssMatch[0]);
      if (css.status !== 200) return;
      text = css.body.toString('utf8');
    }
    if (!text) return;
...
async function refreshChunkCache(html) {
  ...
  for (const u of uniq) {
    try {
      const remote = await fetchRemote('https://www.shader.se' + u);
...
async function fetchAndServePatchedChunk(pathname, query, res) {
  ...
  try {
    const remote = await fetchRemote('https://www.shader.se' + pathname.split('?')[0] + (query || ''));
...
function streamProxy(targetUrl, req, res, pathname) {
  if (STANDALONE) {
    ...
    const body = proxyErrorBody(pathname, 'Yerel dosya bulunamadi: ' + pathname);
    res.writeHead(404, {
...
async function getHomePage() {
  ...
  try {
    const page = await fetchRemote('https://www.shader.se/');
    if (page.status === 200) {
...
const server = http.createServer(async (req, res) => {
  ...
  if (pathname === '/' || pathname === '/index.html') {
    if (isRscRequest(req)) {
      ...
      res.writeHead(404, { 'Content-Type': 'text/plain; charset=utf-8' });
      return res.end('RSC verisi yerelde yok. Calistirin: npm run mirror');
    }
    ...
    return res.end(
      errorPage(
        STANDALONE
          ? 'Yerel site dosyalari eksik. Once: node mirror-assets.js'
          : 'Internet baglantisi gerekli. Wi-Fi acik mi kontrol edin.'
      )
    );
  }
  ...
  return streamProxy(pathname + query, req, res, pathname);
});
...
function onListen() {
  ...
  if (STANDALONE) console.log('    Mod: YEREL (shader.se baglanmaz)');
  console.log('  ====================================');
  console.log('');
  ...
  if (STANDALONE) {
    ...
    console.log(hasStatic ? '  Yerel varliklar hazir.' : '  UYARI: static/ yok — node mirror-assets.js calistirin.');
  }
}
