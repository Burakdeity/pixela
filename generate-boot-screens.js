/** boot_screen — Shader ile ayni duzen, yalnizca PIXELA markasi */
const fs = require('fs');
const path = require('path');
const sharp = require('sharp');
const opentype = require('opentype.js');
const { getBrand } = require('./site-config');
...
async function renderMonoLayer(text, canvasW, canvasH, x, y, size, anchor = 'middle') {
  const svg = `<svg width="${canvasW}" height="${canvasH}" xmlns="http://www.w3.org/2000/svg">
<text x="${x}" y="${y}" fill="#FFFFFF" font-family="'Courier New', Courier, monospace" font-size="${size}" font-weight="700" text-anchor="${anchor}">${esc(text)}</text>
</svg>`;
  return whiteMask(await sharp(Buffer.from(svg)).png().toBuffer());
}
...
async function buildDesktop(font) {
  ...
  // Shader metin bolgeleri — orijinal konumlar
  base = await eraseRect(base, 200, 44, 430, 76);
  base = await eraseRect(base, 120, 152, 480, 36);
  base = await eraseRect(base, 0, 352, meta.width, 24);

  const title = await renderTitleLayer(font, BRAND, meta.width, meta.height, 408, 82, 400, 72);
  const sub = await renderMonoLayer(
    `${BRAND} Development Studio, Website`,
    meta.width,
    meta.height,
    360,
    166,
  ...
  const ver = await renderMonoLayer('Version 1.02', meta.width, meta.height, 360, 182, 13);
  const copy = await renderMonoLayer(
    `Copyright (c) ${BRAND} Development Studio AB, 2026. All Rights Reserved.`,
    meta.width,
    meta.height,
    360,
    370,
...
async function buildMobile(font) {
  ...
  base = await eraseRect(base, 40, 104, 280, 32);
  base = await eraseRect(base, 0, 212, meta.width, 32);

  const title = await renderTitleLayer(font, BRAND, meta.width, meta.height, 248, 57, 120, 44);
  const sub = await renderMonoLayer(`${BRAND} Development Studio`, meta.width, meta.height, 180, 116, 10);
  const ver = await renderMonoLayer('Version 1.02', meta.width, meta.height, 180, 130, 10);
  const copy1 = await renderMonoLayer(`Copyright (c) ${BRAND} Sweden AB, 2026.`, meta.width, meta.height, 180, 228, 9);
  const copy2 = await renderMonoLayer('All Rights Reserved.', meta.width, meta.height, 180, 240, 9);
