...
(function () {
  ...
  const LIDER_VIDEO = '/videos/heip-lider-teknik.mp4?v=94';
  const WP_URL =
    'https://wa.me/905319282677?text=' + encodeURIComponent('Merhaba, PIXELA web sitesinden yazıyorum.');
  const LIDER_SITE = 'https://www.sakaryaliderklima.com/';
  const LIDER_DESC =
  ...
  function rewriteUrl(src) {
    ...
    if (src.includes(LIDER_MUX)) {
      ...
      if (/\/api\/mux-image\//i.test(src)) return '/videos/heip-poster.jpg?v=94';
      return LIDER_VIDEO;
    }
  ...
  function start() {
    ...
    hookWindowOpen();
    patchContactLinks();
    [10000, 20000].forEach((ms) => setTimeout(patchDomTurkish, ms));
    document.documentElement.lang = 'tr';
    document.title = document.title.replace(/Shader Development Studio/i, BRAND + ' — Yazılım & Web');
    document.title = document.title.replace(/PIXELA Development Studio/i, BRAND + ' — Yazılım & Web');
    document.title = document.title.replace(/PIXELA Geliştirme Stüdyosu/i, BRAND + ' — Yazılım & Web');
